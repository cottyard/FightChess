...
(function() {
  var abort_preview, hovering_coord, in_previewing_condition, init, last_mouse_position, launch_preview, on_drop, on_hover, on_mousedown, on_mousemove, on_mouseup, on_pick, paint_previewing_piece, preview, previewing, previewing_coord, previewing_piece, update_mouse_position, view_info, view_info_coord;
  ...
  in_previewing_condition = function() {
    return board.instance.is_occupied(previewing_coord) && board.instance.get_piece(previewing_coord) === previewing_piece;
  };

  preview = function(evt) {
    var mo, moves, mr, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2, _results;
    if (!in_previewing_condition()) {
      abort_preview();
      return;
    }
  ...
  view_info = function(evt) {
    if (board.instance.is_occupied(view_info_coord)) {
      return ui.info.value = board.instance.get_piece(view_info_coord).info();
    } else {
  ...
  on_pick = function(evt) {
    if (!(board.instance.is_occupied(evt.coord) && board.instance.get_piece(evt.coord).can_move())) {
      return;
    }

  ...
  Board = (function() {
    ...
    class Board {
      ...
      set_out_board() {
        ...
        this.spawn();
        this.spawn();
        return this.activate_spawn_cd();
      }

      activate_spawn_cd() {
        return this.spawn_cd = rule.spawn.spawn_cd;
      }

      reduce_spawn_cd() {
        if (this.spawn_cd > 0) {
          return this.spawn_cd--;
        }
      }
      ...
      * all_pieces() {
      ...
      spawn() {
        var col, k, l, len, p, results, row, spawn_cols;
        results = [];
        for (k = 0, len = players.length; k < len; k++) {
          p = players[k];
          spawn_cols = [];
          row = spawn_row[p];
          for (col = l = 1; l <= 8; col = ++l) {
          ...
          results.push(this.place_piece(new piece.Piece(p, 'pawn'), [spawn_cols[calc.randint([0, spawn_cols.length - 1])], row]));
        }
        return results;
      }

...
(function() {
  ...
  handle_assist_queue = function() {
    ...
    while (assist_queue.length > 0) {
      results.push(handle_assist(assist_queue.shift()));
    }
    return results;
  };
  ...
  handle_attack_queue = function() {
    ...
    while (attack_queue.length > 0) {
      results.push(handle_attack(attack_queue.shift()));
    }
    return results;
  };

  handle_move_queue = function() {
    var results;
    results = [];
    while (move_queue.length > 0) {
      results.push(handle_move(move_queue.shift()));
    }
    return results;
  };
  ...
  handle_move = function(evt) {
    return evt.piece.move_to(evt.coord_to);
  };

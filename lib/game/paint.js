...
(function() {
  var background, board, determine_shield_transparency, hp_indicator, mark_assist, mark_attack, mark_destination, mark_grid, move_cd_indicator, paint_grid, piece_at_pos, piece_drawing_types, piece_super_pawn_decorator, piece_with_indicators, shield_indicator;
  ...
  piece_at_pos = function(ctx, piece, [pos_x, pos_y]) {
    ...
    if (piece.type === 'super_pawn') {
      return shape.text(ctx, piece_super_pawn_decorator, pos_x + 8, pos_y - half + 20, null, "18px Courier New");
    }
  };
  ...
  mark_attack = function(ctx, coord_from, coord_to, transparency) {
    var to_x, to_y, x, y;
  ...
  mark_assist = function(ctx, coord_from, coord_to) {
    var to_x, to_y, x, y;
    [[x, y], [to_x, to_y]] = calc.shrink_segment(calc.coord_to_pos(coord_from), calc.coord_to_pos(coord_to));
    shape.save_style(ctx);
  ...
  mark_destination = function(ctx, coord_from, coord_to) {
    var to_x, to_y, x, y;
    if (calc.coord_equal(coord_from, coord_to)) {
      return;
    }
  ...
  window.paint = {background, piece_at_pos, board, paint_grid, mark_grid, mark_attack, mark_assist, mark_destination};

...
(function() {
  var add_defensive_move, add_offensive_move, add_regular_move, bishop, bishop_extra_moves, bishop_moves_orients, cannon, cannon_atk_orients, cannon_moves_deltas_0, cannon_moves_deltas_1, is_in_board, is_in_range, king, king_moves_deltas, knight, knight_moves_deltas, pawn, queen, rook, rook_moves_orients, super_pawn, super_pawn_atk_ast_deltas, super_pawn_moves_deltas, valid_moves,
    indexOf = [].indexOf;
  ...
  bishop = function(color, coord, board) {
    var born_col, born_row, col, defensive, dx, dy, i, j, len, len1, next, offensive, regular, row;
    regular = [];
    offensive = [];
    defensive = [];
    [col, row] = coord;
    ...
    if (born_row[color] === row && indexOf.call(born_col, col) >= 0) {
      for (j = 0, len1 = bishop_extra_moves.length; j < len1; j++) {
        [dx, dy] = bishop_extra_moves[j];
        add_regular_move(regular, [col + dx, row + dy], board);
      }
    }
    return {regular, offensive, defensive};
  ...
  cannon_moves_deltas_0 = [[1, 1], [1, -1], [-1, 1], [-1, -1]];

  cannon_moves_deltas_1 = [[1, 0], [0, 1], [-1, 0], [0, -1]];

  cannon_atk_orients = [[1, 0], [0, 1], [-1, 0], [0, -1]];

  cannon = function(color, coord, board) {
    var bracket, col, dx, dy, i, j, k, len, len1, len2, next, offensive, regular, row;
    regular = [];
    offensive = [];
    [col, row] = coord;
    for (i = 0, len = cannon_moves_deltas_0.length; i < len; i++) {
      [dx, dy] = cannon_moves_deltas_0[i];
      add_regular_move(regular, [col + dx, row + dy], board);
    }
    for (j = 0, len1 = cannon_moves_deltas_1.length; j < len1; j++) {
      [dx, dy] = cannon_moves_deltas_1[j];
      if (add_regular_move(regular, [col + dx, row + dy], board)) {
        add_regular_move(regular, [col + dx * 2, row + dy * 2], board);
      }
    }
    for (k = 0, len2 = cannon_atk_orients.length; k < len2; k++) {
      [dx, dy] = cannon_atk_orients[k];
      bracket = false;
      next = [col, row];
      while (true) {
        next = [next[0] + dx, next[1] + dy];
  ...
  window.rule.move = {
    ...
    strategies: {pawn, super_pawn, knight, bishop, rook, queen, king, cannon}
  };

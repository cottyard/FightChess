...
(function() {
  var arraybuffer_to_obj, coord_equal, coord_one_of, coord_to_pos, copy_array, from_string, get_segment_angle, hoop, obj_to_arraybuffer, pick_one, pos_to_coord, randint, read_from_buffer, remove_item_from_array, set_type, shrink_segment, to_string, unwrap_float_from_arraybuffer, wrap_float_for_arraybuffer, write_buf_to_buf, write_to_buffer;
  ...
  write_to_buffer = function(buffer, pos, content) {
    var view;
  ...
  read_from_buffer = function(buffer, pos) {
    var view;
    view = new Uint16Array(buffer, pos, 1);
    return view[0];
  };

  obj_to_arraybuffer = function(obj) {
    var attr, buffer, p;
  ...
  arraybuffer_to_obj = function(buffer, obj) {
    var attr, content, p, results;
    p = 0;
    results = [];
    for (attr in obj) {
  ...
  write_buf_to_buf = function(buffer_1, buffer_2, from_1, from_2, size) {
    var i, j, ref, results, view_1, view_2;
    view_1 = new Uint8Array(buffer_1);
    view_2 = new Uint8Array(buffer_2);
    results = [];
    for (i = j = 1, ref = size; undefined !== 0 && (1 <= ref ? 1 <= j && j <= ref : 1 >= j && j >= ref); i = 1 <= ref ? ++j : --j) {
  ...
  shrink_segment = function(point_from, point_to, length = 15, direction = 2) {
    var angle, dx, dy, shrink_from, shrink_to, to_x, to_y, x, y;
    [x, y] = point_from;
    [to_x, to_y] = point_to;
  ...
  window.calc = {coord_to_pos, pos_to_coord, coord_equal, coord_one_of, get_segment_angle, shrink_segment, randint, pick_one, remove_item_from_array, copy_array, to_string, from_string, set_type, obj_to_arraybuffer, arraybuffer_to_obj, wrap_float_for_arraybuffer, unwrap_float_from_arraybuffer, write_buf_to_buf, write_to_buffer, read_from_buffer};
